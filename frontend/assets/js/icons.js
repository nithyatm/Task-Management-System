// ============================================================
// TaskFlow Pro - SVG Icon Helper (Lucide icons)
// Usage: icon('name', size)  →  returns SVG string
// Only call inside JS template literals (backtick strings).
// ============================================================

const ICONS = {
  'layout-dashboard': '<rect width="7" height="9" x="3" y="3" rx="1"/><rect width="7" height="5" x="3" y="15" rx="1"/><rect width="7" height="5" x="14" y="3" rx="1"/><rect width="7" height="9" x="14" y="11" rx="1"/>',
...
/**
 * Returns an inline SVG string safe for use inside JS template literals.
 * @param {string} name   - icon key (e.g. 'trash-2')
 * @param {number} size   - width/height in px (default 16)
 * @param {string} color  - optional CSS color string
 */
function icon(name, size, color) {
  size = size || 16;
  const paths = ICONS[name] || '';
  const style = color ? ` style="color:${color}"` : '';
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:middle;flex-shrink:0"${style}>${paths}</svg>`;
}
